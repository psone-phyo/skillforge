...
import { LayoutGrid, Menu, MessageSquare, Home, Book } from 'lucide-vue-next';
...
const activeItemStyles = computed(() => (url: NonNullable<InertiaLinkProps['href']>) =>
...
const mainNavItems: NavItem[] = [
  ...
  { title: t('nav.courses'), href: '/courses', icon: Book },
  { title: t('nav.library'), href: '/my-library', icon: LayoutGrid },
];
...
function switchLocale(code: string) {
  // Use Inertia visit for SPA-friendly navigation; your route redirects back.
  router.visit(`/lang/${code}`, { preserveScroll: true });
}
</script>
                            ...
                            <div class="flex h-full flex-1 flex-col justify-between space-y-4 py-6">
                                <nav class="-mx-3 space-y-1">
                                    <Link v-for="item in mainNavItems" :key="item.title" :href="item.href"
                                        class="flex items-center gap-x-3 rounded-lg px-3 py-2 text-sm font-medium hover:bg-sky-400"
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger :as-child="true">
                            <Button variant="ghost" size="sm"
                                class="h-9 px-3 rounded-md border border-white/10 text-white" title="Language">
                                {{ locale.toUpperCase() }}
                            </Button>
                        </DropdownMenuTrigger>
                            ...
                            <template v-for="lang in languages" :key="lang.code">
                                <button @click="switchLocale(lang.code)"
                                    ...
                                    <span v-if="locale === lang.code"
                                        class="ml-2 text-xs rounded-md px-2 py-0.5 border border-white/10">
                                        Current
                                    </span>
                                </button>
                            </template>
                        ...
                        <DropdownMenuTrigger :as-child="true">
                            <Button variant="ghost" size="icon"
                                ...
                                <Avatar class="size-8 overflow-hidden rounded-full">
                                    <AvatarImage v-if="auth.user.profile_url" :src="auth.user.profile_url"
                                        :alt="auth.user.name" />
                                    <AvatarFallback
